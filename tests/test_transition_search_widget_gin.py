...
from tests.steps import (open_start_widget_nio_page, open_start_widget_ts_page, open_transition_search_widget_ts_page,
                         open_transition_search_widget_nio_page, open_start_widget_gin_page,
...
@allure.epic('OPN')
@allure.story('Переход к поиску в виджете ГИН')
@allure.title('Проверка открытия формы поиска')
@allure.severity(allure.severity_level.BLOCKER)
@pytest.mark.parametrize('browser', CLIENT_BROWSERS)
@pytest.mark.parametrize('device_type', CLIENT_DEVICE_TYPE)
def test_transition_search_widget_nio(make_app: Callable[..., Application], browser: str, device_type: str) -> None:
    ...
    open_transition_search_widget_gin_page(app)
